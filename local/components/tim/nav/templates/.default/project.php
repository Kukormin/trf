...
if (!$projectId)
{
	...
	$tabCode = 'settings';
	$projectHref = \Local\Main\Project::getNewHref();
}
else
{
	$tabs = array(
	...
	$projectHref = \Local\Main\Project::getHref($projectId);
...
$campaigns = \Local\Main\Category::getByProject($projectId);
...
foreach ($tabs as $code => $name)
{
	...
	if ($code == 'cat')
	{
		...
		foreach ($campaigns as $item)
		{
			...
			$href = \Local\Main\Category::getHref($item);
		...
		$href = \Local\Main\Category::getNewHref($projectId);
		?>
		<p>
	...
	elseif ($code == 'links')
	{
		...
		<table id="sitelinks" class="table table-striped table-hover">
			...
			foreach ($sets as $set)
			{
				$href = \Local\Main\Linkset::getHref($set);
				?>
				<tr>
		...
		$href = \Local\Main\Linkset::getAddHref($projectId);
		?>
		<p>
	...
	elseif ($code == 'vcards')
	{
		...
		<table id="vcards" class="table table-striped table-hover">
			...
			{
				$href = \Local\Main\Vcard::getHref($card);
				?>
				<tr>
		...
		$href = \Local\Main\Vcard::getAddHref($projectId);
		?>
		<p>
